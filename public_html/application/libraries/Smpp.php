...
/**
 * [MENTION=194597]Package[/MENTION] net 
 * Class for receiving or sending sms through SMPP protocol. 
 * [MENTION=36223]version[/MENTION] 0.2 
 * [MENTION=78233]author[/MENTION] paladin 
 * [MENTION=23991]sinc[/MENTION]e 04/25/2006 
 * [MENTION=58318]see[/MENTION] [url]http://www.smpp.org/doc/public/index.html[/url] - SMPP 3.4 protocol specification 
 */
class Smpp {
     ...
     * [MENTION=336014]Param[/MENTION] $host - SMSC host name or host IP 
     * [MENTION=336014]Param[/MENTION] $port - SMSC port 
     */
    function init($host, $port = 5016) {
        //internal parameters 
     ...
     * [MENTION=336014]Param[/MENTION] $login - ESME system_id 
     * [MENTION=336014]Param[/MENTION] $port - ESME password 
     ...
     */
    protected function splitMessageString($message, $split, $dataCoding = 0) {
        switch ($dataCoding) {
            case 0:
                ...
                $parts[] = $part;
                return $parts;
            case 8: // UCS2-BE can just use str_split since we send 132 octets per message, which gives a fine split using UCS2
            default:
                return str_split($message, $split);
        }
    }
    ...
    function sendSMSLong($from, $to, $message) {
        ...
        if ($msg_length <= 160):
            ...
            $pdu = pack('a1cca' . (strlen($from) + 1) . 'cca' . (strlen($to) + 1) . 'ccca1a1ccccca' . (strlen($message) + 1), 
                    $this->sms_service_type, $this->sms_source_addr_ton, $this->sms_source_addr_npi, $from, //source_addr 
                    $this->sms_dest_addr_ton, $this->sms_dest_addr_npi, $to, //destination_addr 
                    $this->sms_esm_class, $this->sms_protocol_id, $this->sms_priority_flag, 
                    $this->sms_schedule_delivery_time, $this->sms_validity_period, 
                    $this->sms_registered_delivery_flag, $this->sms_replace_if_present_flag, 
                    $this->sms_data_coding, $this->sms_sm_default_msg_id, strlen($message), //sm_length 
                    $message//short_message 
            );
            $status = $this->sendCommand(0x00000004, $pdu);
            //*/ 
            ...
            //if ($msg_length > 254):
                $doCsms = true;
                $parts = $this->splitMessageString($message, 153, $this->sms_data_coding);
           ...
           // if ($doCsms) {
                ...
                foreach ($parts as $part) {
                    ...
                    $pdu = pack('a1cca' . (strlen($from) + 1) . 'cca' . (strlen($to) + 1) . 'ccca1a1ccccca' . (strlen($part) + 1), 
                            $this->sms_service_type, $this->sms_source_addr_ton, $this->sms_source_addr_npi, $from, //source_addr 
                            $this->sms_dest_addr_ton, $this->sms_dest_addr_npi, $to, //destination_addr 
                            $this->sms_esm_class, $this->sms_protocol_id, $this->sms_priority_flag, 
                            $this->sms_schedule_delivery_time, $this->sms_validity_period, 
                            $this->sms_registered_delivery_flag, $this->sms_replace_if_present_flag, 
                            $this->sms_data_coding, $this->sms_sm_default_msg_id, strlen($part), //sm_length 
                            $part//short_message 
                    );
                    $res = $this->sendCommand(0x00000004, $pdu);
    ...
    /**
     * [MENTION=11885]private[/MENTION] function 
     ...
     * [MENTION=336014]Param[/MENTION] $pdu - deliver PDU from SMSC. 
     ...
     * [MENTION=336014]Param[/MENTION] $command_id - command ID 
     * [MENTION=336014]Param[/MENTION] $pdu - PDU body 
     ...
     * [MENTION=336014]Param[/MENTION] $command_id - command ID 
     * [MENTION=336014]Param[/MENTION] $pdu - PDU body 
     * [MENTION=336014]Param[/MENTION] $seq_number - PDU sequence number 
     */
     ...
     * [MENTION=336014]Param[/MENTION] $seq_number - PDU sequence number 
     * [MENTION=336014]Param[/MENTION] $command_id - PDU command ID 
     * @return PDU status or false on error 
     */
     ...
     * [MENTION=336014]Param[/MENTION] $ar - input array 
     * [MENTION=336014]Param[/MENTION] $maxlen - maximum length to read. 
